    ...
    if (req.method === "PUT") {
      ...
      try {
        ...
        if (!result) {
          return res
            .status(404)
            .json({ message: "Story parameters not found" });
        }
      ...
      if (result.deletedCount === 0) {
        return res.status(404).json({ message: "Story parameters not found" });
      }

      res
        .status(200)
        .json({ message: "Story parameters deleted successfully" });
    } else {
